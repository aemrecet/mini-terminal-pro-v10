...
# LightGBM tercihen kullan, yoksa scikit-learn fallback
USE_LGB = True
try:
...
def _features_and_target(df: pd.DataFrame, cfg: dict):
    ...
    # Ertesi gün yukarı mı? (binary hedef)
...
def train_symbol(symbol: str, cfg: dict):
    """Tek sembol için model eğitir ve models/ içine kaydeder. Path döndürür."""
...
def load_ensemble():
    """models/ klasöründeki tüm .pkl modelleri yükler (bozukları atlar)."""
...
def predict_proba_ensemble(models, X_row: pd.DataFrame) -> float | None:
    """Tek satır özellik için ortalama olasılık döndürür."""
    ...
    for m in models:
        try:
            ...
            if hasattr(m, "predict_proba"):
                ...
                # decision_function → sigmoid yaklaşıkla
