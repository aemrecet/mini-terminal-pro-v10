...
def fetch_bist_all(write_csv=True):
    ...
    if not api:
        raise RuntimeError("FINNHUB_API_KEY yok (.env).")
    ...
    if write_csv:
        ensure_dirs()
        df.to_csv(BIST_CSV, index=False)
    return df
...
def fetch_nasdaq_all(write_csv=True):
    ...
    if write_csv:
        ensure_dirs()
        all_df.to_csv(NASDAQ_CSV, index=False)
    return all_df
...
def get_cached_universes():
    return {
        'bist': cached(BIST_CSV),
        'sp500': cached(SP500_CSV),
        'nasdaq': cached(NASDAQ_CSV),
    }
